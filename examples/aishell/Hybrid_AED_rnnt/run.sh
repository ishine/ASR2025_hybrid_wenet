...
train_set=train
train_config=conf/hyb_conformer_u2pp_rnnt.yaml
dir=exp/hyb_conformer_rnnt
checkpoint=
...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
  ...
  ../s0/local/download_and_untar.sh ${data} ${data_url} data_aishell
  ../s0/local/download_and_untar.sh ${data} ${data_url} resource_aishell
fi

if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  # Data preparation
  ../s0/local/aishell_data_prep.sh ${data}/data_aishell/wav \
