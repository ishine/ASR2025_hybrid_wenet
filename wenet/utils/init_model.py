...
##############################################
# revised by wenjieRen
from wenet.hybrid_aed_transducer.hyb_transducer import HybTransducer 
from wenet.hybrid_aed_transducer.hyb_joint import HybTransducerJoint
from wenet.hybrid_aed_transducer.hyb_predictor import (HybConvPredictor, HybEmbeddingPredictor, HybRNNPredictor)
##############################################
...
WENET_PREDICTOR_CLASSES = {
    ...
    ##############################################
    # revised by wenjieRen
    "hyb_rnn": HybRNNPredictor,
    "hyb_embedding": HybEmbeddingPredictor,
    "hyb_conv": HybConvPredictor,
    ##############################################
}
...
WENET_JOINT_CLASSES = {
    ...
    ##############################################
    # revised by wenjieRen
    "hyb_transducer_joint": HybTransducerJoint,
    ##############################################
}
...
def init_speech_model(args, configs):
    ...
    if model_type == "transducer":
        ...
        model = WENET_MODEL_CLASSES[model_type](
            ...
            **configs['model_conf'])
    elif model_type == "hyb_transducer":
        predictor_type = configs.get('predictor', 'rnn')
        joint_type = configs.get('joint', 'hyb_transducer_joint')
        predictor = WENET_PREDICTOR_CLASSES[predictor_type](
            vocab_size, **configs['predictor_conf'])
        joint = WENET_JOINT_CLASSES[joint_type](vocab_size,
                                                **configs['joint_conf'])
